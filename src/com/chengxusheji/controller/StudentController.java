...
public class StudentController extends BaseController {
	...
	public void list(String studentNumber,String studentName,@ModelAttribute("studentClassNumber") ClassInfo studentClassNumber,String studentBirthday,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (studentNumber == null) studentNumber = "";
		if (studentName == null) studentName = "";
		if (studentBirthday == null) studentBirthday = "";
		if(rows != 0)studentService.setRows(rows);
		List<Student> studentList = studentService.queryStudent(studentNumber, studentName, studentClassNumber, studentBirthday, page);
	    /*计算总的页数和总的记录数*/
	    studentService.queryTotalPageAndRecordNumber(studentNumber, studentName, studentClassNumber, studentBirthday);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String studentNumber,String studentName,@ModelAttribute("studentClassNumber") ClassInfo studentClassNumber,String studentBirthday,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (studentNumber == null) studentNumber = "";
		if (studentName == null) studentName = "";
		if (studentBirthday == null) studentBirthday = "";
		List<Student> studentList = studentService.queryStudent(studentNumber, studentName, studentClassNumber, studentBirthday, currentPage);
	    /*计算总的页数和总的记录数*/
	    studentService.queryTotalPageAndRecordNumber(studentNumber, studentName, studentClassNumber, studentBirthday);
	    /*获取到总的页码数目*/
	    int totalPage = studentService.getTotalPage();
	    ...
	    request.setAttribute("studentClassNumber", studentClassNumber);
	...
	public void OutToExcel(String studentNumber,String studentName,@ModelAttribute("studentClassNumber") ClassInfo studentClassNumber,String studentBirthday, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(studentNumber == null) studentNumber = "";
        if(studentName == null) studentName = "";
        if(studentBirthday == null) studentBirthday = "";
        List<Student> studentList = studentService.queryStudent(studentNumber,studentName,studentClassNumber,studentBirthday);
        ...
        for(int i=0;i<studentList.size();i++) {
        	...
        	dataset.add(new String[]{student.getStudentNumber(),student.getStudentName(),student.getStudentSex(),student.getStudentClassNumber().getClassName(),student.getStudentBirthday(),student.getStudentState(),student.getStudentPhoto(),student.getStudentTelephone(),student.getStudentEmail(),student.getStudentQQ()});
        }
        /*
